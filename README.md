# World Models for Autonomous Driving - Deep Learning Short Story

**Name:** Aniket Anil Naik
**Course:** CMPE 258 - Deep Learning, San José State University
**Paper:** "The Role of World Models in Shaping Autonomous Driving: 
           A Comprehensive Survey" (arXiv:2502.10498, updated Feb 2026)

---

## 📖 Medium Article
🔗 [Link coming soon]

## 🖥️ Slide Deck (SlideShare)
🔗 [Link coming soon]

## 🎥 Video Presentation (YouTube)
🔗 [Link coming soon]

## 💻 Code Reproduction
See `/code/reproduction/` — trajectory forecasting experiment 
using the autoresearch template.

---

## 📁 Repo Structure
| Folder | Contents |
|--------|----------|
| `paper/` | Paper reading notes and breakdown |
| `code/` | Autoresearch-based reproduction experiment |
| `article/` | Medium article draft |
| `slides/` | Presentation deck (PDF) |
| `video/` | YouTube link and recording notes |
| `assets/` | Figures and diagrams |

---

## 📚 Paper Reference
Tu, S. et al. "The Role of World Models in Shaping Autonomous 
Driving: A Comprehensive Survey." arXiv:2502.10498 (2025, 
updated Feb 2026).
